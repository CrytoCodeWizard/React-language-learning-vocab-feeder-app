  ...
  (async () => {
    ...
    for (const action in actionVals) {
      slackApp.action(actionVals[action], async ({ body, ack, client }) => {
        ...
        const keyStringByActionId = await storage.getItem(KEYSTRING_BY_ACTION_ID)
        const currentKeyString = keyStringByActionId[body.actions[0].action_id]
        const timestampByKeystring = await storage.getItem(TIMESTAMP_BY_KEYSTRING);
...
const buildPronunciationString = (pronunciationlink) => (pronunciationlink === EMPTY_PRONUNCIATION_LINK ? NO_URL_FOUND_STRING : (`<${pronunciationlink}|(Pronunciation)>`))
...
const getRowIdFromSlackAction = (payloadActionId) => parseInt(payloadActionId.split('-')[1])
...
const buildUpdatedDutchBlockStr = (actionId, actionValue, keyString, keyStringData) => {
  ...
  for (const entryIndex in keyStringData) {
    if (!slackVars.masteredIds.includes(keyStringData[entryIndex].id) && !slackVars.seenIds.includes(keyStringData[entryIndex].id) && rowIdOfAction !== keyStringData[entryIndex].id) {
      ...
      const actionStr = slackVars.masteredIds.includes(keyStringData[entryIndex].id) ? MASTERED_STRING : SEEN_STRING
      blockStr.push({
        type: SLACK_BLOCK_TYPE_SECTION,
        fields: [{
          type: SLACK_BLOCK_FIELD_TYPE_MARKDOWN,
